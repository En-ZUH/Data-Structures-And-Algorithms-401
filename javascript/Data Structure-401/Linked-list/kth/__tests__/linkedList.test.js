...
describe('Test append, insertAfter, insertBefore methods', () => {
    ...
    test('Can successfully insert a node before the first node of a linked list.', () => {
        ...
        list.insertBefore('first', 'bataineh');



        expect(list.toString()).toEqual('{ bataineh } -> { first } -> { 30 } -> { 20 } -> { 10 } -> { name } -> { enas } -> { 20 } -> { 10 } -> { 25 } -> { 4 } -> { 3 } -> { 2 } -> { 1 } -> { 4 } -> { 3 } -> { 2 } -> { 1 } -> { 100 } -> { 10 } -> { 1 } -> { 7 } -> { 5 } -> { 10 } -> { 0 } -> { 10 } -> { 20 } -> { 30 } -> {40} -> NULL');
